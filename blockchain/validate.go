...
const (
	...
	// baseSubsidy is the starting block reward in atoms.
	// FalconDraw: 1000 FDRW at genesis, halving every 210000 blocks.
...
// should have, in atoms (1 FDRW = 1e8 atoms).
//
// FalconDraw emission: max(1, floor(1000 / 2^(floor(H/210000)))) FDRW per block.
// Floor is applied in whole-FDRW units before converting to atoms, so the
// minimum reward is exactly 1 FDRW (1e8 atoms) forever — no decay, no burn.
func CalcBlockSubsidy(height int32, chainParams *chaincfg.Params) int64 {
	...
	// Apply floor() in whole-FDRW units as the spec requires.
	fdrw := int64(1000) >> halvings
	if fdrw < 1 {
		fdrw = 1
	}
	return fdrw * btcutil.SatoshiPerBitcoin
}
...
func checkProofOfWork(header *wire.BlockHeader, blockHeight int32, powLimit *big.Int, flags BehaviorFlags) error {
	...
	if flags&BFNoPoWCheck != BFNoPoWCheck {
		hash := BlockPoWHash(header)
		hashNum := HashToBig(&hash)
		if hashNum.Cmp(target) > 0 {
			str := fmt.Sprintf("block pow hash of %064x is higher than "+
...
func CheckProofOfWork(block *btcutil.Block, powLimit *big.Int) error {
	return checkProofOfWork(&block.MsgBlock().Header, block.Height(), powLimit, BFNone)
}
...
func CheckBlockHeaderSanity(header *wire.BlockHeader, blockHeight int32,
	powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {

	// Ensure the proof of work bits in the block header is in min/max range
	// and the block hash is less than the target value described by the
	// bits.
	err := checkProofOfWork(header, blockHeight, powLimit, flags)
	if err != nil {
		return err
	}
...
func checkBlockSanity(block *btcutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
	...
	err := CheckBlockHeaderSanity(header, block.Height(), powLimit, timeSource, flags)
	if err != nil {
		return err
	}
...
func (b *BlockChain) checkBlockContext(block *btcutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
	...
	if !fastAdd {
		...
		if segwitState == ThresholdActive {
			...
			// consensus parameter. Settlement blocks use a higher
			// limit to accommodate the draw settlement transaction,
			// which has one input per staking wallet.
			blockWeight := GetBlockWeight(block)
			maxWeight := int64(MaxBlockWeight)
			if blockContainsVDFResult(block) {
				// VDF result block: allow extra weight for VDFResult tx
				// (~370 bytes) and draw settlement tx.
				maxWeight = MaxVDFBlockWeight
			}
...
func CheckTransactionInputs(tx *btcutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	// Pool draw settlement and VDFResult transactions have sentinel inputs that
	// are not real UTXOs — skip UTXO lookups for both. Fee is always zero.
	if IsDrawSettlementTx(tx) || IsVDFResultTx(tx) {
		return 0, nil
	}
...
func (b *BlockChain) checkConnectBlock(node *blockNode, block *btcutil.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
	...
	for _, tx := range transactions {
		...
		// Validate pool staking transactions: format, draw-height, and balance.
		if IsPoolStakingTx(tx) {
			if err := ValidatePoolStakeTx(tx, node.height, view); err != nil {
				return err
			}
		}

		// Draw settlement TXs require a VDFResult tx in the same block.
		// The full cross-tx check is done after the per-tx loop below.
	...
	// VDF settlement validation: find the VDFResult tx in this block (if any),
	// verify the Sloth proof, run the draw inline, and validate the settlement tx.
	// Settlement and VDFResult always land in the same block.
	// Returns the VDF bounty amount paid to the submitter (0 when no draw occurred).
	vdfBounty, err := b.validateVDFSettlement(node, block, view)
	if err != nil {
		return err
	}
...
// blockContainsVDFResult returns true if any transaction in block has
// TxVersionVDFResult.  Used for block weight limit selection.
func blockContainsVDFResult(block *btcutil.Block) bool {
	for _, tx := range block.Transactions() {
		if tx.MsgTx().Version == wire.TxVersionVDFResult {
			return true
		}
	}
	return false
...
// validateVDFSettlement enforces VDF + settlement consensus rules for a block:
//
//   - Find any VDFResult tx in the block.
//   - If found: verify the Sloth proof, run the draw inline with the VDF seed,
//     and validate the settlement tx structure and payout.
//   - If NOT found: reject any draw settlement tx in the block.
// validateVDFSettlement returns the VDF bounty amount that was paid (> 0 only when
// there is an actual draw with pool balance > 0), so the caller can add it to
// the expected coinbase value check.
func (b *BlockChain) validateVDFSettlement(node *blockNode, block *btcutil.Block, view *UtxoViewpoint) (int64, error) {
	vdfParams := NewVDFParams(chaincfg.VDFPrime)

	var vdfDrawHeight uint32
	var vdfDrawSeed [32]byte
	var vdfSubmitterWalletHash [20]byte
	vdfFound := false
	...
	for _, tx := range block.Transactions() {
		...
		drawHeight, drawBlockHashArr, T, vdfOutput, submitterWalletHash, err := ExtractVDFResultData(tx)
		if err != nil {
			return 0, ruleError(ErrBadVDFResultTx,
				fmt.Sprintf("invalid VDFResult tx %v: %v", tx.Hash(), err))
		}
		// T must meet the minimum.
		if T < chaincfg.VDFMinT {
			return 0, ruleError(ErrBadVDFResultTx,
				fmt.Sprintf("VDFResult T=%d below minimum %d", T, chaincfg.VDFMinT))
		}
		// drawHeight must be a valid draw block height.
		if !IsDrawBlock(int32(drawHeight)) {
			return 0, ruleError(ErrBadVDFResultTx,
				fmt.Sprintf("VDFResult drawHeight %d is not a draw block", drawHeight))
		}
		// drawHeight must be below the current block.
		if int32(drawHeight) >= node.height {
			return 0, ruleError(ErrBadVDFResultTx,
				fmt.Sprintf("VDFResult drawHeight %d >= block height %d", drawHeight, node.height))
		}
		// The draw block hash in the tx must match the chain.
		drawNode := node.Ancestor(int32(drawHeight))
		if drawNode == nil {
			return 0, ruleError(ErrBadVDFResultTx,
				fmt.Sprintf("VDFResult drawHeight %d not in ancestor chain", drawHeight))
		}
		if drawNode.hash != drawBlockHashArr {
			return 0, ruleError(ErrBadVDFResultTx,
				fmt.Sprintf("VDFResult drawBlockHash mismatch at height %d", drawHeight))
		}
		// Post-activation, T is consensus-fixed: it must equal the value
		// derived from the draw block's own ancestor window. Otherwise the
		// submitter picks T freely, and since the draw seed depends on the
		// VDF output — itself a function of T — every candidate T is an
		// independent roll of the draw. Gated on drawHeight (not this
		// block's height) so a draw's rule never changes mid-settlement
		// when the settlement block lands after the flag day.
		if act := b.chainParams.VDFExactTActivationHeight; act != 0 &&
			int32(drawHeight) >= act {
			if expected := b.expectedVDFT(drawNode); T != expected {
				return 0, ruleError(ErrBadVDFResultTx,
					fmt.Sprintf("VDFResult T=%d for draw %d does not match "+
						"consensus-expected T=%d", T, drawHeight, expected))
			}
		}
		// Draw must not already be settled -- but only if the recorded
		// settlement is actually an ancestor of this block. FetchVDFResult
		// reads from the db, which reflects whichever chain is currently
		// connected; during a reorg check (getReorganizeNodes/attachNodes)
		// that can still be the *old* chain even though we're validating a
		// block on a competing branch. Without this ancestry check, a draw
		// settled by the old (soon to be disconnected) chain would
		// permanently poison the new chain's equally-valid settlement
		// block as invalid, and since the block index marks it and all its
		// descendants invalid forever, the node would never be able to
		// accept that branch again -- even after the old chain is
		// disconnected.
		settlementHeight, settlementHash, hasHash, _, _, alreadySettled := b.fetchVDFSettlement(drawHeight)
		if alreadySettled {
			switch {
			case hasHash:
				// New-format record: only a settlement that is an ancestor
				// of this block counts as a duplicate.
				if ancestor := node.Ancestor(settlementHeight); ancestor != nil && ancestor.hash.IsEqual(&settlementHash) {
					return 0, ruleError(ErrBadVDFResultTx,
						fmt.Sprintf("VDFResult for draw %d already settled", drawHeight))
				}
			case settlementHeight < b.vdfFormatCutoff:
				// Legacy pre-cutoff record: grandfathered under the old
				// rule -- any recorded settlement is a duplicate. Safe
				// because pre-cutoff history is not subject to live reorgs.
				return 0, ruleError(ErrBadVDFResultTx,
					fmt.Sprintf("VDFResult for draw %d already settled (legacy record)", drawHeight))
			default:
				// A hashless record at or above the cutoff violates the
				// format invariant -- post-cutoff code always writes the
				// hash. Reject as settled (the old, stricter behavior) and
				// log loudly so the corruption gets noticed.
				log.Errorf("VDF: settlement record for draw %d at height %d "+
					"has no block hash but is above format cutoff %d -- "+
					"possible db corruption", drawHeight, settlementHeight,
					b.vdfFormatCutoff)
				return 0, ruleError(ErrBadVDFResultTx,
					fmt.Sprintf("VDFResult for draw %d already settled (malformed record)", drawHeight))
			}
		}
		// Verify the Sloth proof (T squarings, ~3.5 s on reference VPS).
		var seedArr [32]byte
		copy(seedArr[:], drawBlockHashArr[:])
		if !SlothVerify(seedArr, vdfOutput, T, vdfParams) {
			return 0, ruleError(ErrBadVDFResultTx,
				fmt.Sprintf("VDFResult Sloth proof invalid for draw %d", drawHeight))
		}
		// Derive the draw seed.
		var drawBlockCH chainhash.Hash
		copy(drawBlockCH[:], drawBlockHashArr[:])
		vdfDrawSeed = DrawSeed(drawBlockCH, vdfOutput)
		vdfDrawHeight = drawHeight
		vdfSubmitterWalletHash = submitterWalletHash
		vdfFound = true
		break // only one VDFResult per block
	}

	if vdfFound {
		// Run the draw inline using the VDF seed.
		stakes := b.FetchPoolStakeEntries(vdfDrawHeight)
		result := RunDraw(stakes, vdfDrawSeed)
		pd := PoolDrawData{
			Balance:    b.ReconstructPoolBalance(vdfDrawHeight),
			JackpotWon: result.JackpotWon,
		}
		if result.JackpotWon && len(result.Winners) > 0 {
			pd.WinnerWalletHash = result.Winners[0].WalletHash
		}

		// Enforce VDF bounty only when there is an actual draw (pool balance > 0).
		// Skip when submitter hash is all-zeros (tests / simnet) or no pool balance.
		vdfBounty := int64(0)
		var zeroHash [20]byte
		if pd.Balance > 0 && vdfSubmitterWalletHash != zeroHash {
			vdfBounty = CalcVDFBounty(node.height)
			wantScript := MakeWinnerP2WPKHScript(vdfSubmitterWalletHash)
			coinbase := block.Transactions()[0].MsgTx()
			found := false
			for _, out := range coinbase.TxOut {
				if out.Value == vdfBounty && bytes.Equal(out.PkScript, wantScript) {
					found = true
					break
				}
			}
			if !found {
				return 0, ruleError(ErrBadCoinbaseValue,
					fmt.Sprintf("VDF result block at height %d missing VDF bounty output of %d atoms to submitter",
						node.height, vdfBounty))
			}
		}

		if err := ValidateVDFDrawTxs(block, vdfDrawHeight, view, pd); err != nil {
			return 0, err
		}
		return vdfBounty, nil
	}

	// No VDFResult in block — settlement txs are forbidden.
	for _, tx := range block.Transactions() {
		if IsDrawSettlementTx(tx) {
			return 0, ruleError(ErrSettlementTxWithoutVDFResult,
				fmt.Sprintf("draw settlement tx %v requires VDFResult tx in same block (height %d)",
					tx.Hash(), node.height))
		}
	}
	return 0, nil
