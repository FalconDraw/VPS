...
var rpcHandlersBeforeInit = map[string]commandHandler{
	...
	"getdrawinfo":            handleGetDrawInfo,
	"getdrawresult":          handleGetDrawResult,
	"getdrawsizeinfo":        handleGetDrawSizeInfo,
	"getdrawtickets":         handleGetDrawTickets,
	"getdrawstakers":         handleGetDrawStakers,
	"getstakerstats":         handleGetStakerStats,
	"getmempoolstakes":       handleGetMempoolStakes,
	"getaddressbalance":      handleGetAddressBalance,
	"getwalletstakehistory":  handleGetWalletStakeHistory,
	"getaddressincoming":     handleGetAddressIncoming,
...
func (state *gbtWorkState) blockTemplateResult(useCoinbaseValue bool, submitOld *bool) (*btcjson.GetBlockTemplateResult, error) {
	...
	// data, then include the witness commitment in the GBT result. Per the
	// Bitcoin Core convention that mining software (e.g. ckpool) expects,
	// this is the full coinbase output script -- OP_RETURN OP_DATA_36
	// followed by the witness magic bytes and the commitment hash -- not
	// just the bare hash. This must match the scriptPubKey actually built
	// in AddWitnessCommitment above byte-for-byte.
	if template.WitnessCommitment != nil {
		fullCommitment := append(append([]byte{}, blockchain.WitnessMagicBytes...),
			template.WitnessCommitment...)
		reply.DefaultWitnessCommitment = hex.EncodeToString(fullCommitment)
	}
...
// handleGetAddressUTXOs returns all unspent outputs for an address in a single
// server-side call, avoiding the N sequential gettxout round trips the client
...
func handleGetAddressUTXOs(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	addrHash := addr.ScriptAddress() // 20-byte pubkey hash

	// Collect all (txid, vout_index, amount, script, isCoinbase) from the
	// address index in one DB transaction. We don't check UTXO state here
	// because FetchUtxoEntry needs the chain lock, not a DB transaction.
	...
	err = s.cfg.DB.View(func(dbTx database.Tx) error {
		...
		for {
			...
			if err != nil {
				// Block may be an orphan from a reorg whose addrindex entry
				// was never cleaned up — skip rather than fail the whole scan.
			...
			for _, serialized := range serializedTxns {
				var msgTx wire.MsgTx
				if err := msgTx.Deserialize(bytes.NewReader(serialized)); err != nil {
					continue
				}
				txHash := msgTx.TxHash()
				cb := blockchain.IsCoinBaseTx(&msgTx)
				for vout, out := range msgTx.TxOut {
					script := out.PkScript
					// P2PKH: OP_DUP OP_HASH160 OP_DATA20 <hash20> OP_EQUALVERIFY OP_CHECKSIG
					if len(script) == 25 &&
						script[0] == 0x76 && script[1] == 0xa9 && script[2] == 0x14 &&
						script[23] == 0x88 && script[24] == 0xac &&
						bytes.Equal(script[3:23], addrHash) {
						candidates = append(candidates, candidate{
							txid: txHash, vout: uint32(vout),
							amount: out.Value, script: script, isCoinbase: cb,
						})
						continue
					}
					// P2WPKH: OP_0 OP_DATA20 <hash20>
					if len(script) == 22 &&
						script[0] == 0x00 && script[1] == 0x14 &&
						bytes.Equal(script[2:22], addrHash) {
						candidates = append(candidates, candidate{
							txid: txHash, vout: uint32(vout),
							amount: out.Value, script: script, isCoinbase: cb,
						})
	...
	// Scan mempool: collect unconfirmed outputs to this address and build
	// the set of outpoints currently being spent by mempool transactions.
	// Confirmed UTXOs in mempoolSpent are excluded below so the wallet
	// never sees a UTXO as spendable while its spend is still pending.
	mempoolSpent := make(map[wire.OutPoint]struct{})
	for _, txDesc := range s.cfg.TxMemPool.TxDescs() {
		msgTx := txDesc.Tx.MsgTx()
		for _, in := range msgTx.TxIn {
			mempoolSpent[in.PreviousOutPoint] = struct{}{}
		}
		for vout, out := range msgTx.TxOut {
			script := out.PkScript
			// P2PKH
			if len(script) == 25 &&
				...
				bytes.Equal(script[3:23], addrHash) {
				candidates = append(candidates, candidate{
					txid: *txDesc.Tx.Hash(), vout: uint32(vout),
					amount: out.Value, script: script,
				})
				continue
			}
			// P2WPKH
	...
	// Check UTXO set for each candidate.
	best := s.cfg.Chain.BestSnapshot()
	var utxos []btcjson.AddressUTXO
	for _, c := range candidates {
		outPoint := wire.OutPoint{Hash: c.txid, Index: c.vout}
		if _, spent := mempoolSpent[outPoint]; spent {
			continue // pending spend in mempool — not spendable yet
		}
		entry, err := s.cfg.Chain.FetchUtxoEntry(outPoint)
		if err != nil || entry == nil || entry.IsSpent() {
			// Also check mempool (unconfirmed)
			if s.cfg.TxMemPool.HaveTransaction(&c.txid) {
				tx, err := s.cfg.TxMemPool.FetchTransaction(&c.txid)
				if err != nil {
					continue
				}
				msgTx := tx.MsgTx()
				if int(c.vout) >= len(msgTx.TxOut) {
					continue
				}
				utxos = append(utxos, btcjson.AddressUTXO{
					TxID:          c.txid.String(),
					Vout:          c.vout,
					Atoms:         msgTx.TxOut[c.vout].Value,
					Confirmations: 0,
					Script:        hex.EncodeToString(c.script),
					IsCoinbase:    c.isCoinbase,
					IsStaking:     c.isStaking,
					DrawHeight:    c.drawHeight,
				})
			}
			continue
		}
		confs := best.Height - entry.BlockHeight() + 1
		if confs < 0 {
			confs = 0
		}
		utxos = append(utxos, btcjson.AddressUTXO{
			...
			Atoms:         entry.Amount(),
			Confirmations: confs,
			Script:        hex.EncodeToString(entry.PkScript()),
			IsCoinbase:    entry.IsCoinBase(),
			IsStaking:     c.isStaking,
			DrawHeight:    c.drawHeight,
		})
	...
	if utxos == nil {
		utxos = []btcjson.AddressUTXO{}
	}
	return utxos, nil
}

// handleGetAddressBalance implements the getaddressbalance command. It returns
// a spendable/staked summary for an address in one call.
func handleGetAddressBalance(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	// Scan UTXOs — reuse the same candidate-collect + UTXO-check pattern.
	type candidate struct {
		txid       chainhash.Hash
		vout       uint32
		amount     int64
	...
	err = s.cfg.DB.View(func(dbTx database.Tx) error {
		...
		for {
			...
			serializedTxns, err := dbTx.FetchBlockRegions(regions)
			if err != nil {
				// Stale addrindex entry from an orphaned block — skip.
				skip += uint32(len(regions))
				if uint32(len(regions)) < batchSize {
					break
				}
			...
			for _, serialized := range serializedTxns {
				...
				for vout, out := range msgTx.TxOut {
					...
					if len(script) == 25 &&
						...
						candidates = append(candidates, candidate{
							txid: txHash, vout: uint32(vout),
							amount: out.Value, isCoinbase: cb,
						})
						continue
					}
					// P2WPKH
	...
	// Mempool unconfirmed P2PKH and P2WPKH outputs.
	for _, txDesc := range s.cfg.TxMemPool.TxDescs() {
		msgTx := txDesc.Tx.MsgTx()
		for vout, out := range msgTx.TxOut {
			script := out.PkScript
			...
			if len(script) == 25 &&
				...
				bytes.Equal(script[3:23], addrHash) {
				candidates = append(candidates, candidate{
					txid: *txDesc.Tx.Hash(), vout: uint32(vout), amount: out.Value,
				})
				continue
			}
			// P2WPKH
	...
	for _, cand := range candidates {
		op := wire.OutPoint{Hash: cand.txid, Index: cand.vout}
		entry, err := s.cfg.Chain.FetchUtxoEntry(op)
		unconfirmed := false
		if err != nil || entry == nil || entry.IsSpent() {
			if !s.cfg.TxMemPool.HaveTransaction(&cand.txid) {
				continue
			}
			unconfirmed = true
		}
		...
		if cand.isStaking {
			staked += cand.amount
		} else if !cand.isCoinbase || confs >= 100 {
			spendable += cand.amount
		}
	}

	total := spendable + staked
	return &btcjson.GetAddressBalanceResult{
		Address:       c.Address,
		SpendableFDRW: btcutil.Amount(spendable).ToBTC(),
		StakedFDRW:    btcutil.Amount(staked).ToBTC(),
		TotalFDRW:     btcutil.Amount(total).ToBTC(),
	}, nil
}
...
func fetchInputTxos(s *rpcServer, tx *wire.MsgTx) (map[wire.OutPoint]wire.TxOut, error) {
	...
	for txInIndex, txIn := range tx.TxIn {
		// Sentinel inputs (pool settlement txs) are synthetic — they have no
		// backing UTXO in the index. Skip them; callers handle missing prevOuts.
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if len(addressTxns) < numRequested {
		err = s.cfg.DB.View(func(dbTx database.Tx) error {
			...
			if err != nil {
				// Stale addrindex entry from an orphaned block — skip
				// these entries rather than failing the whole call.
				return nil
			}
...
type rpcServer struct {
	...
	quit                   chan int
	// drawResultCache memoizes getdrawresult responses keyed by draw height
	// (uint32). Settled draw results are immutable so entries never expire.
	drawResultCache sync.Map
}
...
func createMarshalledReply(rpcVersion btcjson.RPCVersion, id interface{}, result interface{}, replyErr error) ([]byte, error) {
	...
	// When the client (e.g. ckpool) sends no jsonrpc version field, default
	// to 1.0 so MarshalResponse doesn't reject the empty string.
	if rpcVersion == "" {
...
func (s *rpcServer) processRequest(request *btcjson.Request, isAdmin bool, closeChan <-chan struct{}) []byte {
	...
	if jsonErr == nil {
		...
		// per the JSON-RPC spec. When rpcquirks is enabled, respond anyway
		// (matches Bitcoin Core behaviour — required for ckpool compatibility).
...
func (s *rpcServer) Start() {
	...
	// Pre-warm drawResultCache for ALL settled draws so the first dashboard
	// request finds everything cached. The dashboard now fetches the full
	// history, so every draw height needs to be in the cache at startup.
	// Runs concurrently with node startup; errors are silently ignored.
	go func() {
		best := s.cfg.Chain.BestSnapshot()
		lastDraw := uint32(best.Height) / uint32(blockchain.DrawInterval) * uint32(blockchain.DrawInterval)
		closeChan := make(chan struct{})
		for dh := lastDraw; dh >= uint32(blockchain.DrawInterval); dh -= uint32(blockchain.DrawInterval) {
			handleGetDrawResult(s, btcjson.NewGetDrawResultCmd(dh), closeChan)
		}
	}()
