...
type BlockChain struct {
	...
	// drawSeqCache memoizes CountSettledDraws results keyed by draw height
	// (uint32). Settled draws never change, so cached values are permanently
	// valid. sync.Map is used so concurrent RPC goroutines can read without
	// blocking each other.
	drawSeqCache sync.Map

	// VDF computation state.  pendingVDF holds the result of the background
	// Sloth evaluation once it completes.  vdfCancel cancels any in-progress
	// computation (e.g., on reorg).  vdfCurrentHeight is the draw height the
	// goroutine is currently computing (0 = none).  All fields protected by vdfMu.
	vdfMu                  sync.RWMutex
	pendingVDF             *PendingVDFResult
	vdfCancel              context.CancelFunc
	vdfCurrentHeight       uint32
	vdfStartTime           time.Time // when the current VDF goroutine started
	vdfSubmitterWalletHash [20]byte  // set from Config.VDFSubmitterWalletHash at startup
	vdfMsPerStep           float64   // measured ms/SlothEval-step; 0 = not yet benchmarked. A property
	// of this machine, not of any one draw — never reset on reorg/cancel, only ever overwritten
	// by a fresh benchmark at the start of the next computation.

	// vdfProgressPct reports 10%-increment progress (0,10,...,90) of the
	// in-flight SlothEval, written by the computation goroutine itself. Lock-free
	// since it's a single atomic counter, not part of the vdfMu-guarded group above.
	vdfProgressPct atomic.Uint32

	// vdfFormatCutoff is the settlement height at which the 300-byte VDF
	// record format (with settlement block hash) became mandatory. Records
	// below it may be legacy 268-byte entries written by pre-upgrade code;
	// records at or above it must carry a hash. Persisted once at first
	// startup of hash-aware code (see New) and immutable afterwards.
	vdfFormatCutoff int32
}
...
func (b *BlockChain) connectBlock(node *blockNode, block *btcutil.Block,
	...
	err = b.db.Update(func(dbTx database.Tx) error {
		...
		// Update the staking UTXO index for draw consensus.
		err = connectBlockStakingIndex(dbTx, block, stxos)
		if err != nil {
			return err
		}
	...
	// Start background VDF computation after a draw block connects — but only
	// when real stakers are present.  A rollover-only draw (no stakers, just
	// jackpot balance) rolls over without winner selection, so VDF is not
	// needed and would waste compute that could interfere with a prior draw's
	// in-flight computation.
	if IsDrawBlock(node.height) {
		stakes, _ := b.FetchDrawStakes(uint32(node.height))
		if len(stakes) > 0 {
			b.startVDFComputation(uint32(node.height), node.hash)
		}
	}
	// Clear pending VDF once the VDF+settlement block lands, then check for
	// any subsequent draws that still need VDF computation (e.g., if a draw
	// block connected while the previous draw's VDF was still running).
	if blockContainsVDFResult(block) {
		b.vdfMu.Lock()
		b.pendingVDF = nil
		b.vdfMu.Unlock()
		go b.startNextPendingVDF(uint32(node.height))
	}

	return nil
}

// startNextPendingVDF looks for the oldest unsettled draw block after
// afterHeight that has real stakers, and starts VDF computation for it.
// Called after a VDFResult settlement block connects to pick up any draw that
// was skipped because VDF was already in-flight for an older draw.
// If no next draw is found and the current goroutine is for a draw already
// settled by a peer, it is cancelled and pendingVDF is cleared.
func (b *BlockChain) startNextPendingVDF(afterHeight uint32) {
	bestHeight := b.BestSnapshot().Height
	for dh := afterHeight + uint32(DrawInterval); int32(dh) <= bestHeight; dh += uint32(DrawInterval) {
		_, _, _, settled := b.FetchVDFResult(dh)
		if settled {
			continue
		}
		stakes, _ := b.FetchDrawStakes(dh)
		if len(stakes) == 0 {
			continue
		}
		h, err := b.BlockHashByHeight(int32(dh))
		if err != nil {
			return
		}
		log.Infof("VDF chain: starting computation for draw %d after draw %d settled", dh, afterHeight)
		b.startVDFComputation(dh, *h)
		return
	}
	// No next pending draw found.  If the current goroutine is still running
	// for a draw that a peer has already settled, cancel it and clear any
	// stale pendingVDF it may have written after we cleared it above.
	b.vdfMu.RLock()
	curDH := b.vdfCurrentHeight
	b.vdfMu.RUnlock()
	if curDH == 0 {
		return
	}
	if _, _, _, alreadySettled := b.FetchVDFResult(curDH); alreadySettled {
		b.vdfMu.Lock()
		if b.vdfCancel != nil {
			b.vdfCancel()
			b.vdfCancel = nil
		}
		b.vdfCurrentHeight = 0
		b.pendingVDF = nil
		b.vdfMu.Unlock()
		log.Infof("VDF: cancelled stale goroutine for draw %d (already settled by peer)", curDH)
	}
}
...
func (b *BlockChain) disconnectBlock(node *blockNode, block *btcutil.Block, view *UtxoViewpoint) error {
	...
	err = b.db.Update(func(dbTx database.Tx) error {
		...
		// Update the staking UTXO index for draw consensus.
		// VDFResult tx disconnect reverses draw result meta and rollover.
		if err := disconnectBlockStakingIndex(dbTx, block, stxos); err != nil {
			return err
		}
	...
	// On reorg: cancel any in-flight VDF computation for the disconnected
	// draw block and clear the pending result.
	if IsDrawBlock(node.height) {
		b.vdfMu.Lock()
		if b.vdfCancel != nil {
			b.vdfCancel()
...
// Pool draw sentinel inputs and VDFResult sentinel inputs are excluded because
// they are virtual (no real UTXO).
func countSpentOutputs(block *btcutil.Block) int {
	// Exclude the coinbase transaction since it can't spend anything.
	var numSpent int
	for _, tx := range block.Transactions()[1:] {
		// VDFResult txs have no real inputs.
		if IsVDFResultTx(tx) {
			continue
		}
...
func (b *BlockChain) ReconsiderBlock(hash *chainhash.Hash) error {
	...
	if err != nil {
		...
		// that it's invalid.
		log.Warnf("Reconsidered branch failed reorg verification "+
			"(detach %d, attach %d): %v", detachNodes.Len(),
			attachNodes.Len(), err)
		return nil
	}
...
type Config struct {
	...
	// VDFSubmitterWalletHash is the 20-byte HASH160 of the node operator's
	// public key.  It is embedded in VDFResult txs so the consensus layer
	// can verify the VDF bounty coinbase output in settlement blocks.
	// Leave as zero to disable bounty (e.g. simnet, tests).
	VDFSubmitterWalletHash [20]byte
}
...
func New(config *Config) (*BlockChain, error) {
	...
	b := BlockChain{
		...
		vdfSubmitterWalletHash: config.VDFSubmitterWalletHash,
	}
	...
	// Read-only audit of the pool-balance index. The old startup repair
	// mutated balances based on restart timing and desynced nodes from the
	// network; rollover of stranded balances is handled deterministically
	// at settlement by dbAbsorbIntermediateBalances.
	if err := b.AuditPoolBalances(); err != nil {
		return nil, err
	}

	// Load the VDF record format cutoff, persisting it on first startup of
	// hash-aware code: everything settled up to the current tip is
	// grandfathered legacy format; from the next block on, settlement
	// records must carry the settlement block hash.
	if err := b.db.Update(func(dbTx database.Tx) error {
		cutoff := dbFetchVDFFormatCutoff(dbTx)
		if cutoff < 0 {
			cutoff = int32(bestNode.height) + 1
			if err := dbPutVDFFormatCutoff(dbTx, cutoff); err != nil {
				return err
			}
			log.Infof("VDF: record format cutoff initialized at height %d", cutoff)
		}
		b.vdfFormatCutoff = cutoff
		return nil
	}); err != nil {
		return nil, err
	}

	// Resume VDF computation for any draw whose block has already passed but
	// whose VDF result has not yet been stored.  Walk backward from the most
	// recently completed draw block until we find one with stakers, then
	// restart the background goroutine.  Most of the time this is a no-op
	// (VDF already settled), but after a crash or restart mid-computation it
	// picks up where the node left off.
	go func() {
		tipHeight := int32(bestNode.height)
		for dh := tipHeight / int32(DrawInterval) * int32(DrawInterval); dh >= int32(DrawInterval); dh -= int32(DrawInterval) {
			_, _, _, settled := b.FetchVDFResult(uint32(dh))
			if settled {
				break // already settled — nothing to resume
			}
			stakes, _ := b.FetchDrawStakes(uint32(dh))
			if len(stakes) == 0 {
				continue // no stakers — VDF not needed for this draw
			}
			// Found an unsettled draw with stakers.  Get its block hash and start VDF.
			h, err := b.BlockHashByHeight(dh)
			if err != nil {
				log.Warnf("VDF restart: cannot get hash for draw block %d: %v", dh, err)
				break
			}
			log.Infof("VDF restart: resuming Sloth computation for draw block %d", dh)
			b.startVDFComputation(uint32(dh), *h)
			break
		}
	}()

	// Pre-warm drawSeqCache in the background so the first RPC call for draw
	// results doesn't stall waiting for a full genesis-to-tip scan. The scan
	// runs concurrently with the rest of node startup and is harmless if the
	// chain has no settled draws yet.
	go func() {
		lastDraw := uint32(bestNode.height) / uint32(DrawInterval) * uint32(DrawInterval)
		if lastDraw >= uint32(DrawInterval) {
			b.CountSettledDraws(lastDraw) //nolint:errcheck
		}
	}()
...
// GetPendingVDFResult returns the completed VDF result for the most recent draw
// block, or nil if computation is still in progress or no draw block has
// connected yet.  Safe for concurrent access.
func (b *BlockChain) GetPendingVDFResult() *PendingVDFResult {
	b.vdfMu.RLock()
	defer b.vdfMu.RUnlock()
	return b.pendingVDF
}

// GetVDFStartTime returns when the current VDF computation started.
// The zero value means no computation has been started yet.
func (b *BlockChain) GetVDFStartTime() time.Time {
	b.vdfMu.RLock()
	defer b.vdfMu.RUnlock()
	return b.vdfStartTime
}

// GetVDFProgressPct returns the last-recorded 10%-increment progress (0-90)
// of the in-flight VDF computation. Meaningless once computation completes —
// callers should treat a settled/pending result as the 100% signal instead.
func (b *BlockChain) GetVDFProgressPct() uint32 {
	return b.vdfProgressPct.Load()
}

// GetVDFMsPerStep returns the most recently benchmarked real-world ms/step
// for this machine, or an 11ms bootstrap default if this node has never run
// a VDF benchmark yet (e.g. no draw with stakers has connected since startup).
func (b *BlockChain) GetVDFMsPerStep() float64 {
	b.vdfMu.RLock()
	defer b.vdfMu.RUnlock()
	if b.vdfMsPerStep <= 0 {
		return 11.0
	}
	return b.vdfMsPerStep
}

// expectedVDFT is the consensus-expected VDF step count for a draw block,
// active for draws at or above VDFExactTActivationHeight. It must be a pure
// function of the draw block's ancestor chain — every node computes it
// independently and validateVDFSettlement rejects a VDFResult whose committed
// T differs — so it uses only int64 arithmetic over blockNode timestamps
// (no floats, no hardware benchmark):
//
//	T = VDFTargetBlocks × avgBlockSecs × 1000 / VDFReferenceMsPerStep
//
// avgBlockSecs is the mean spacing of the VDFTWindowBlocks blocks ending at
// the draw block, clamped to [TargetTimePerBlock/10, 4×TargetTimePerBlock].
// A full draw interval's window smooths LWMA-3's burst/stall oscillation —
// the old 5-block window sampled a single phase of that cycle, sizing T off
// block times anti-correlated with the ones that followed (T swung 728 to
// 367,300 within a day of LWMA-3 activating). The clamp bounds how far a
// miner can shrink T by pulling the draw block's timestamp toward its
// median-time-past floor, and keeps T sane across regime changes.
func (b *BlockChain) expectedVDFT(drawNode *blockNode) uint64 {
	oldHeight := drawNode.height - chaincfg.VDFTWindowBlocks
	if oldHeight < 0 {
		oldHeight = 0
	}
	spanBlocks := int64(drawNode.height - oldHeight)
	oldNode := drawNode.Ancestor(oldHeight)
	if oldNode == nil || spanBlocks <= 0 {
		return chaincfg.VDFInitialT
	}

	spanSecs := drawNode.timestamp - oldNode.timestamp
	targetSecs := int64(b.chainParams.TargetTimePerBlock / time.Second)
	if minSpan := spanBlocks * targetSecs / 10; spanSecs < minSpan {
		spanSecs = minSpan
	}
	if maxSpan := spanBlocks * targetSecs * 4; spanSecs > maxSpan {
		spanSecs = maxSpan
	}
...
// CalcNextVDFT computes T so the VDF takes approximately VDFTargetBlocks
// worth of wall-clock time.
//
// For draws at or above VDFExactTActivationHeight this returns expectedVDFT —
// the consensus-fixed value that validateVDFSettlement enforces — computed
// against the main-chain node at drawHeight, or the current tip when the draw
// block doesn't exist yet (pre-draw RPC estimates).
//
// Below activation it keeps the legacy heuristic: T derived from the average
// block time of the 5 blocks immediately prior to the draw block and this
// machine's own benchmarked per-step cost:
//
//	T = VDFTargetBlocks × avgBlockSecs × (1000 / msPerStep)
//
// Returns VDFInitialT if block timestamps are unavailable (early chain).
func (b *BlockChain) CalcNextVDFT(drawHeight uint32) uint64 {
	if act := b.chainParams.VDFExactTActivationHeight; act != 0 &&
		int32(drawHeight) >= act {
		node := b.bestChain.NodeByHeight(int32(drawHeight))
		if node == nil {
			// Draw block not connected yet — estimate off the tip's
			// window. Only used for RPC display; the value is recomputed
			// against the real draw block before evaluation.
			node = b.bestChain.Tip()
		}
		if node == nil {
			return chaincfg.VDFInitialT
		}
		return b.expectedVDFT(node)
	}

	msPerStep := b.GetVDFMsPerStep()
	const lookback = int32(5)

	best := b.BestSnapshot().Height
	curHeight := int32(drawHeight)
	if best < curHeight {
		curHeight = best // draw block doesn't exist yet — use chain tip
	}
	oldHeight := curHeight - lookback
	if oldHeight < 0 {
		oldHeight = 0
	}
	...
	curBlock, err1 := b.BlockByHeight(curHeight)
	oldBlock, err2 := b.BlockByHeight(oldHeight)
	if err1 != nil || err2 != nil {
		return chaincfg.VDFInitialT
	}

	spanSecs := curBlock.MsgBlock().Header.Timestamp.Sub(
		oldBlock.MsgBlock().Header.Timestamp).Seconds()
	spanBlocks := float64(curHeight - oldHeight)
	if spanSecs <= 0 || spanBlocks <= 0 {
		return chaincfg.VDFInitialT
	}

	avgBlockSecs := spanSecs / spanBlocks
	newT := uint64(float64(chaincfg.VDFTargetBlocks) * avgBlockSecs * 1000 / msPerStep)
	if newT < chaincfg.VDFMinT {
		newT = chaincfg.VDFMinT
	}
	return newT
}

// startVDFComputation launches a background goroutine that runs SlothEval over
// drawBlockHash and stores the result in b.pendingVDF when complete.  Any
// previously in-flight computation is cancelled first.
func (b *BlockChain) startVDFComputation(drawHeight uint32, drawBlockHash chainhash.Hash) {
	b.vdfMu.Lock()
	if b.vdfCancel != nil {
		b.vdfCancel()
	}
	...
	b.vdfCancel = cancel
	b.vdfCurrentHeight = drawHeight
	b.pendingVDF = nil
	b.vdfMu.Unlock()
	b.vdfProgressPct.Store(0)

	startTime := time.Now()

	b.vdfMu.Lock()
	b.vdfStartTime = startTime
	b.vdfMu.Unlock()

	go func() {
		// Pin to OS thread so the goroutine isn't migrated mid-computation.
		runtime.LockOSThread()
		defer runtime.UnlockOSThread()

		vdfParams := NewVDFParams(chaincfg.VDFPrime)

		// Fresh hardware benchmark before picking T — real per-step cost on
		// this machine right now, not a hardcoded assumption. Runs inside the
		// goroutine (not synchronously in startVDFComputation) so it never
		// blocks connectBlock's chainLock; ~2s is negligible next to the
		// multi-minute computation it calibrates.
		const vdfBenchmarkWindow = 2 * time.Second
		if ms := vdfBenchmarkMsPerStep(vdfParams, vdfBenchmarkWindow); ms > 0 {
			b.vdfMu.Lock()
			b.vdfMsPerStep = ms
			b.vdfMu.Unlock()
		}
		...
		if act := b.chainParams.VDFExactTActivationHeight; act != 0 &&
			int32(drawHeight) >= act {
			// Consensus-fixed T: compute against the exact draw block being
			// evaluated (by hash, not best-chain height) so the result stays
			// valid even if the best chain reorganizes around this height
			// while the computation is queued.
			if node := b.index.LookupNode(&drawBlockHash); node != nil {
				T = b.expectedVDFT(node)
			} else {
				T = b.CalcNextVDFT(drawHeight)
			}
		} else {
			T = b.CalcNextVDFT(drawHeight)
		}

		log.Infof("VDF: starting computation for draw %d (T=%d)", drawHeight, T)
		var seed [32]byte
		copy(seed[:], drawBlockHash[:])
		output := SlothEval(ctx, seed, T, vdfParams, &b.vdfProgressPct)
		if output == nil {
			log.Infof("VDF: computation cancelled for draw %d", drawHeight)
			return
		}
		drawSeed := DrawSeed(drawBlockHash, output)
		computeSecs := int64(time.Since(startTime).Seconds())
		result := &PendingVDFResult{
			...
			SubmitterWalletHash: b.vdfSubmitterWalletHash,
		}
		// If a peer published and settled this draw while we were computing,
		// discard the result so the miner doesn't see stale pendingVDF data.
		if _, _, _, alreadySettled := b.FetchVDFResult(drawHeight); alreadySettled {
			log.Infof("VDF: draw %d already settled by peer; discarding local result after %ds", drawHeight, computeSecs)
			return
		}
		// Verify the draw block hash still matches the chain.  A reorg that
		// replaced block drawHeight after VDF started would leave us with a
		// hash mismatch that causes every block template to fail validation.
		currentHash, err := b.BlockHashByHeight(int32(drawHeight))
		if err != nil || *currentHash != drawBlockHash {
			log.Infof("VDF: draw %d hash mismatch after computation (reorg?); discarding result", drawHeight)
			return
		}
		b.vdfMu.Lock()
		b.pendingVDF = result
		b.vdfMu.Unlock()
		log.Infof("VDF: computation complete for draw %d in %ds", drawHeight, computeSecs)
	}()
}
