...
var (
	...
	// mainPowLimit is the highest proof of work value (maximum target) a
	// FalconDraw block can have for the main network. Set to match the
	// genesis block bits (0x1f0fffff): 0x0fffff << 224. This means network
	// difficulty can never drop below the genesis level.
	mainPowLimit = new(big.Int).Lsh(new(big.Int).SetInt64(0x0fffff), 224)
	...
	// VDFPrime is the 2048-bit safe prime used for Sloth VDF modular square
	// roots.  This is RFC 3526 MODP Group 14 (p = 2^2048 - 2^1984 + ... + 11).
	// It satisfies p ≡ 3 (mod 4), which is required for efficient square roots.
	// Hardcoded so every node uses the same modulus from genesis.
	VDFPrime = func() *big.Int {
...
const (
	// VDFInitialT is the starting number of Sloth iterations.  Set low for
	// debug/testnet so VDF completes in ~1 s; raise toward 100_000 for mainnet
	// (~18 min at ~11 ms/step) once block time stabilises.
	VDFInitialT = uint64(100)

	// VDFMinT is the minimum T accepted in a VDFResult tx.  Prevents a node
	// from submitting a trivially fast result.
	VDFMinT = uint64(10)

	// VDFTargetBlocks is the target number of blocks between the draw block
	// and the VDFResult block.  At steady-state 10-min blocks this equals
	// 30 min of VDF work, which makes miner grinding impossible.  During
	// early chain (fast blocks) T stays small so VDF still arrives ~3
	// blocks later; as difficulty pushes block time toward 10 min, T grows
	// proportionally until VDF converges on ~30 real minutes.
	VDFTargetBlocks = int64(3)

	// VDFAdjustInterval is the number of settled draws used as the rolling
	// window for the T calibration.
	VDFAdjustInterval = 2016

	// VDFTWindowBlocks is the trailing block window, ending at the draw
	// block, whose average spacing sizes the consensus-expected T once
	// VDFExactTActivationHeight is active. One full draw interval: long
	// enough to smooth LWMA-3's burst/stall oscillation (a short window
	// samples only one phase of the cycle and is anti-correlated with the
	// blocks that follow), short enough to track genuine pace changes
	// within about one draw.
	VDFTWindowBlocks = int32(36)

	// VDFReferenceMsPerStep is the fixed per-step cost (milliseconds) used
	// in the consensus ExpectedVDFT formula. The live hardware benchmark
	// each node runs before computing (GetVDFMsPerStep) cannot participate
	// in consensus — every machine measures differently — so the expected
	// step count is defined against this reference machine instead. Faster
	// hardware simply finishes the same T early. Integer so the whole
	// formula stays in int64 arithmetic (bit-identical on every platform).
	VDFReferenceMsPerStep = int64(11)
)
...
type Params struct {
	...
	// EmergencyAdjustWindow is FalconDraw's one-way emergency difficulty
	// adjustment: for a block that isn't at a normal BlocksPerRetarget
	// boundary, if it arrives more than EmergencyAdjustWindow after its
	// parent, required difficulty eases by a factor of
	// RetargetAdjustmentFactor for every additional full window elapsed,
	// compounding, until a block lands. Zero disables it.
	//
	// Deliberately one-directional: this only ever eases difficulty down.
	// Recovery back up happens solely through the normal BlocksPerRetarget
	// cycle, never through this rule — avoiding the boom-bust oscillation
	// exploit seen in Bitcoin Cash's 2017 EDA, where a symmetric fast-react
	// rule let miners swarm in after a difficulty crash and leave once it
	// snapped back up.
	//
	// The number of windows counted from a single block-to-block timestamp
	// gap is capped (see calcNextRequiredDifficulty) so a miner can't claim
	// an outsized one-shot difficulty crash by setting a timestamp near the
	// MaxTimeOffsetSeconds future-drift limit. A genuine multi-hour drought
	// still fully recovers via compounding across the sequence of blocks
	// that land during it.
	EmergencyAdjustWindow time.Duration

	// EmergencyAdjustActivationHeight is the flag-day height at which the
	// EmergencyAdjustWindow ease-down becomes part of consensus. Blocks
	// below this height validate under the pre-emergency rule (difficulty
	// carries over unchanged between BlocksPerRetarget boundaries
	// regardless of block spacing). Without this gate, a fresh node
	// re-validating history rejects any pre-deployment block that spanned
	// a quiet gap, because the ease-down retroactively changes the
	// expected difficulty for blocks mined before the rule existed.
	//
	// Zero means active from genesis (the behavior before this field
	// existed), so Params that don't set it are unaffected.
	EmergencyAdjustActivationHeight int32

	// LWMA3ActivationHeight is the flag-day height at which the chain
	// switches from the legacy BlocksPerRetarget/EmergencyAdjustWindow
	// difficulty scheme to the continuous LWMA-3 retarget (see
	// calcNextRequiredDifficultyLWMA3 in blockchain/difficulty.go). At and
	// after this height, calcNextRequiredDifficulty recalculates every
	// block using the trailing LWMA3WindowSize-block window instead of only
	// at BlocksPerRetarget boundaries, replacing the emergency one-way
	// ease-down entirely (LWMA-3 corrects both directions).
	//
	// Zero disables it entirely (same "zero disables it" convention as
	// EmergencyAdjustWindow, not BIP0034Height's "always active" — needed
	// so unset test-network Params keep their existing legacy/
	// ReduceMinDifficulty behavior instead of silently switching to
	// LWMA-3 from genesis). Follows the BIP0034Height-style flag-day
	// pattern rather than BIP0009 voting since this is an unconditional
	// height switch, not a miner-signaled rule.
	LWMA3ActivationHeight int32

	// VDFExactTActivationHeight is the flag-day height at which the VDF
	// step count T becomes consensus-fixed: for draws at or above this
	// height, a VDFResult tx is only valid if its committed T equals
	// ExpectedVDFT (blockchain/chain.go) — a deterministic function of the
	// draw block's trailing VDFTWindowBlocks timestamps. Below this height
	// the legacy rule applies (any T >= VDFMinT).
	//
	// Without this rule the submitter chooses T freely, and because the
	// draw seed is DrawSeed(drawBlockHash, vdfOutput) with vdfOutput a
	// function of T, each candidate T yields an independent draw outcome:
	// anyone could grind T = VDFMinT, VDFMinT+1, ... (each trivially cheap
	// to evaluate at small T) until the draw pays their own wallet, then
	// submit that result. Fixing T to a chain-derived value removes the
	// degree of freedom entirely; the only remaining grind is re-mining
	// the draw block itself, which is what the VDF's multi-block wall-clock
	// cost is designed to make unprofitable.
	//
	// Zero disables it (same convention as LWMA3ActivationHeight), keeping
	// unset test-network Params on the legacy rule.
	VDFExactTActivationHeight int32
...
var MainNetParams = Params{
	...
	Net:         wire.FalconNet,
	DefaultPort: "8333",
	// FalconDraw has no DNS seed infrastructure yet.
	// Peers connect via explicit addpeer or the public VPS address.
	DNSSeeds: []DNSSeed{},
	...
	EmergencyAdjustWindow: time.Minute * 10, // one-way difficulty ease-down when blocks stall
	// The ease-down was deployed 2026-07-08 with both nodes at height
	// 40094; blocks at/above 40095 were mined and accepted under it,
	// everything below was mined under the carry-over rule.
	EmergencyAdjustActivationHeight: 40095,
	// Flag day for the LWMA-3 retarget switch-over — set 2026-07-10 during
	// the incident where a hashrate recovery after the EmergencyAdjustWindow
	// eased difficulty down left it stuck near the floor (one-way ease-down,
	// only the slow 2016-block cycle corrects upward). Height chosen as
	// tip (49318) + ~5000 blocks for deploy/test buffer, not a round number
	// tied to anything else.
	LWMA3ActivationHeight: 54500,
	// Flag day for the consensus-fixed VDF T — set 2026-07-13 after finding
	// that validation only enforced T >= VDFMinT (10 steps), leaving the
	// draw seed grindable by whoever submits the VDFResult (see the field's
	// doc comment). Height chosen as tip (54912) + ~530 blocks: ~3.5 days
	// at the 10-minute LWMA-3 target, buffer for both nodes to deploy.
	VDFExactTActivationHeight: 55440,
	GenerateSupported:         false,

	// No checkpoints yet — FalconDraw is a new chain.
	Checkpoints: nil,
	...
	// BIP 173.
	Bech32HRPSegwit: "fdrw", // FalconDraw mainnet
...
var RegressionNetParams = Params{
	...
	// Human-readable part for Bech32 encoded segwit addresses, as defined in
	// BIP 173.
	Bech32HRPSegwit: "rfdrw", // FalconDraw regtest
...
var TestNet3Params = Params{
	...
	// Human-readable part for Bech32 encoded segwit addresses, as defined in
	// BIP 173.
	Bech32HRPSegwit: "tfdrw", // FalconDraw testnet
...
var SimNetParams = Params{
	...
	// Human-readable part for Bech32 encoded segwit addresses, as defined in
	// BIP 173.
	Bech32HRPSegwit: "sfdrw", // FalconDraw simnet
