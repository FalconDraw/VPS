...
type Config struct {
	...
	// FetchConfirmedStakeCount returns the number of confirmed pool stake
	// transactions already indexed for a (drawHeight, walletHash) pair.
	// Used together with the in-memory pendingStakeCounts to enforce
	// MaxStakesPerWalletPerDraw before a transaction enters the mempool.
	// May be nil (limit is skipped when unset).
	FetchConfirmedStakeCount func(drawHeight uint32, walletHash [20]byte) int
}
...
type TxPool struct {
	...
	// pendingStakeCounts tracks how many pool stake transactions are
	// currently pending in the mempool per (drawHeight, walletHash).
	// Keyed as pendingStakeCounts[drawHeight][walletHash].
	// Protected by mtx.
	pendingStakeCounts map[uint32]map[[20]byte]int
}
...
func (mp *TxPool) removeTransaction(tx *btcutil.Tx, removeRedeemers bool) {
	...
	if txDesc, exists := mp.pool[*txHash]; exists {
		...
		// Decrement pending stake count.
		if txDesc.Tx.MsgTx().Version == wire.TxVersionPoolStake {
			if _, script := blockchain.FindPoolStakingOpReturn(txDesc.Tx.MsgTx()); script != nil {
				if wh, _, dh, err := blockchain.ExtractPoolStakingData(script); err == nil {
					if mp.pendingStakeCounts[dh] != nil {
						mp.pendingStakeCounts[dh][wh]--
						if mp.pendingStakeCounts[dh][wh] <= 0 {
							delete(mp.pendingStakeCounts[dh], wh)
						}
					}
				}
			}
...
// EvictExpiredPoolStakes removes all pool staking transactions (version 6)
// whose target draw height is <= currentHeight. Called on every block connect
// so stale stakes don't accumulate in the mempool indefinitely.
//
// This function is safe for concurrent access.
func (mp *TxPool) EvictExpiredPoolStakes(currentHeight int32) {
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
	for _, desc := range mp.pool {
		tx := desc.Tx
		if !blockchain.IsPoolStakingTx(tx) {
			continue
		}
		_, script := blockchain.FindPoolStakingOpReturn(tx.MsgTx())
		if script == nil {
			continue
		}
		_, _, dh, err := blockchain.ExtractPoolStakingData(script)
		if err == nil && int32(dh) <= currentHeight {
			log.Debugf("Evicting expired pool stake %s (drawHeight=%d, current=%d)",
				tx.Hash(), dh, currentHeight)
...
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint, tx *btcutil.Tx, height int32, fee int64) *TxDesc {
	...
	// Track pending stake count for per-wallet limit enforcement.
	if tx.MsgTx().Version == wire.TxVersionPoolStake {
		if _, script := blockchain.FindPoolStakingOpReturn(tx.MsgTx()); script != nil {
			if wh, _, dh, err := blockchain.ExtractPoolStakingData(script); err == nil {
				if mp.pendingStakeCounts[dh] == nil {
					mp.pendingStakeCounts[dh] = make(map[[20]byte]int)
				}
				mp.pendingStakeCounts[dh][wh]++
			}
		}
...
func (mp *TxPool) checkMempoolAcceptance(tx *btcutil.Tx,
	...
	// Enforce per-wallet pool stake limit. O(1): one map lookup + one
	// optional DB read, both under the mempool lock.
	if tx.MsgTx().Version == wire.TxVersionPoolStake && mp.cfg.FetchConfirmedStakeCount != nil {
		if _, script := blockchain.FindPoolStakingOpReturn(tx.MsgTx()); script != nil {
			if wh, _, dh, err := blockchain.ExtractPoolStakingData(script); err == nil {
				confirmed := mp.cfg.FetchConfirmedStakeCount(dh, wh)
				pending := 0
				if mp.pendingStakeCounts[dh] != nil {
					pending = mp.pendingStakeCounts[dh][wh]
				}
				if confirmed+pending >= blockchain.MaxStakesPerWalletPerDraw {
					return nil, txRuleError(wire.RejectDuplicate,
						fmt.Sprintf("wallet already has %d stakes in draw %d (limit %d)",
							confirmed+pending, dh, blockchain.MaxStakesPerWalletPerDraw))
				}
			}
		}
	}
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		pendingStakeCounts: make(map[uint32]map[[20]byte]int),
	}
}
