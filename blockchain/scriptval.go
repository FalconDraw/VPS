...
out:
	for {
		...
		case txVI := <-v.validateChan:
			...
			txIn := txVI.txIn
			// Pool draw settlement TXs and VDFResult TXs use sentinel inputs
			// with no real UTXOs — skip script validation for both.
			if IsPoolDrawSentinelInput(txIn) || IsVDFResultTx(txVI.tx) {
				v.sendResult(nil)
				continue
			}
			...
			ver := txVI.tx.MsgTx().Version

			// TxVersionPoolStake inputs carry the Schnorr sig in the witness
			// field (SegWit discount). Sentinel inputs have no sig to verify.
			if ver == wire.TxVersionPoolStake {
				if IsPoolDrawSentinelInput(txIn) {
					v.sendResult(nil)
					continue
				}
				sigHash := StakingTxSigHash(txVI.tx.MsgTx())
				pubKey, err := VerifyPoolStakingSchnorrWitness(txIn.Witness, sigHash)
				if err != nil {
					v.sendResult(ruleError(ErrScriptValidation,
						fmt.Sprintf("pool staking tx %s input %d: %v",
							txVI.tx.Hash(), txVI.txInIndex, err)))
					break out
				}
				// Enforce that the signing pubkey owns the UTXO being spent.
				// Only P2PKH and P2WPKH UTXOs are valid pool staking inputs.
				spenderHash := pubKeyHash(pubKey.SerializeCompressed())
				var utxoHash [20]byte
				switch {
				case len(pkScript) == 25 && pkScript[0] == 0x76 && pkScript[1] == 0xa9 &&
					pkScript[2] == 0x14 && pkScript[23] == 0x88 && pkScript[24] == 0xac:
					copy(utxoHash[:], pkScript[3:23]) // P2PKH
				case len(pkScript) == 22 && pkScript[0] == 0x00 && pkScript[1] == 0x14:
					copy(utxoHash[:], pkScript[2:22]) // P2WPKH
				default:
					v.sendResult(ruleError(ErrScriptValidation,
						fmt.Sprintf("pool staking tx %s input %d: UTXO script type not supported (only P2PKH/P2WPKH)",
							txVI.tx.Hash(), txVI.txInIndex)))
					break out
				}
				if spenderHash != utxoHash {
					v.sendResult(ruleError(ErrScriptValidation,
						fmt.Sprintf("pool staking tx %s input %d: witness pubkey does not own the UTXO",
							txVI.tx.Hash(), txVI.txInIndex)))
					break out
				}
				v.sendResult(nil)
...
func ValidateTransactionScripts(tx *btcutil.Tx, utxoView *UtxoViewpoint,
	...
	// validation.
	// VDFResult txs have no real inputs to validate.
	if IsVDFResultTx(tx) {
		return nil
	}
