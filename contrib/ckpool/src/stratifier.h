...
struct genwork {
	...
	bool insert_witness;
	/* FalconDraw: set when this workbase's transaction set includes a
	 * VDFResult (tx version 7) transaction, which requires the coinbase
	 * to also pay a VDF bounty output to vdf_bounty_hash. */
	bool insert_vdf_bounty;
	uchar vdf_bounty_hash[20];
	int merkles;
