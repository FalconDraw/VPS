...
const (
	...
	// FalconDraw uses 5MB blocks: MaxBlockWeight = 4 × MaxBlockBaseSize = 20,000,000.
	MaxBlockWeight = 20_000_000

	// MaxVDFBlockWeight is the maximum block weight for a VDF result block —
	// the block containing the VDFResult tx and draw settlement tx for a draw.
	// The higher limit provides headroom for the VDFResult tx (~370 bytes) plus
	// the settlement tx and any expired winner-claim sweep inputs.
	MaxVDFBlockWeight = 256_000_000

	// MaxBlockBaseSize is the maximum number of bytes within a block
	// which can be allocated to non-witness data. FalconDraw uses 5MB blocks.
	MaxBlockBaseSize = 5_000_000

	// MaxBlockSigOpsCost is the maximum number of signature operations
	// allowed for a block. Scaled 5× from Bitcoin's 80,000 to match the
	// 5× larger block size.
	MaxBlockSigOpsCost = 400_000
