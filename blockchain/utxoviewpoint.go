...
func (view *UtxoViewpoint) connectTransaction(tx *btcutil.Tx, blockHeight int32, stxos *[]SpentTxOut) error {
	...
	// VDFResult TXs have a sentinel input with no corresponding UTXO.
	if IsVDFResultTx(tx) {
		view.AddTxOuts(tx, blockHeight)
		return nil
	}
	...
	for _, txIn := range tx.MsgTx().TxIn {
		// Pool draw settlement TXs use a virtual sentinel input that has no
		// corresponding UTXO — skip it.
...
func (view *UtxoViewpoint) disconnectTransactions(db database.DB, block *btcutil.Block, stxos []SpentTxOut) error {
	...
	for txIdx := len(transactions) - 1; txIdx > -1; txIdx-- {
		...
		for txInIdx := len(tx.MsgTx().TxIn) - 1; txInIdx > -1; txInIdx-- {
			// Pool draw settlement and VDFResult sentinel inputs have no UTXO/STXO entry.
			if IsPoolDrawSentinelInput(tx.MsgTx().TxIn[txInIdx]) || IsVDFResultTx(tx) {
				continue
			}
...
func (view *UtxoViewpoint) findInputsToFetch(block *btcutil.Block) []wire.OutPoint {
	...
	for i, tx := range transactions[1:] {
		// Sentinel-input transactions (settlement, VDFResult) have no real UTXOs to fetch.
...
// FetchUtxoEntries fetches multiple UTXO entries in a single chain lock
// acquisition and a single DB transaction for cache misses. Callers must not
// modify the returned entries.
func (b *BlockChain) FetchUtxoEntries(outpoints []wire.OutPoint) ([]*UtxoEntry, error) {
	b.chainLock.RLock()
	defer b.chainLock.RUnlock()
	return b.utxoCache.fetchEntries(outpoints)
}
