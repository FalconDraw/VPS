...
type serverPeer struct {
	...
	*peer.Peer
	...
	// peerAdded is set by handleAddPeerMsg after syncManager.NewPeer
	// is called. handleDonePeerMsg reads this to decide whether to
	// notify the sync manager and evict orphans: those side effects
	// must only run for peers that were actually registered via
	// syncManager.NewPeer.
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	// Derive VDF submitter wallet hash from the first configured mining address.
	// This is embedded in v7 (VDFResult) txs so validators can enforce the bounty
	// coinbase output in settlement blocks.
	var vdfSubmitterWalletHash [20]byte
	if len(cfg.miningAddrs) > 0 {
		if wp, ok := cfg.miningAddrs[0].(interface{ WitnessProgram() []byte }); ok {
			prog := wp.WitnessProgram()
			if len(prog) == 20 {
				copy(vdfSubmitterWalletHash[:], prog)
			}
		}
	}
	...
	s.chain, err = blockchain.New(&blockchain.Config{
		...
		VDFSubmitterWalletHash: vdfSubmitterWalletHash,
	})
	if err != nil {
		return nil, err
	}
	...
	txC := mempool.Config{
		...
		FetchConfirmedStakeCount: s.chain.FetchWalletStakeCount,
	...
	if cfg.GenThreads > 0 {
		s.cpuMiner.SetNumWorkers(int32(cfg.GenThreads))
	}
