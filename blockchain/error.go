...
const (
	...
	// ErrBadDrawSettlementTx indicates a draw settlement transaction is
	// structurally invalid — bad marker, wrong draw height, non-staking
	// inputs, conservation-of-value failure, wrong position in the block,
	// or duplicate settlement transactions.
	ErrBadDrawSettlementTx

	// ErrSettlementTxWithoutVDFResult indicates a draw settlement transaction
	// was found in a block that contains no VDFResult tx. A settlement tx is
	// only valid when accompanied by the VDFResult tx in the same block.
	ErrSettlementTxWithoutVDFResult

	// ErrMissingDrawSettlementTx indicates that a VDF result block contains
	// no draw settlement transaction despite a non-zero pool balance for the
	// draw. A settlement tx is required in the same block as the VDFResult tx
	// whenever any wallet staked for that draw.
	ErrMissingDrawSettlementTx

	// ErrBadVDFResultTx indicates a VDFResult tx (v7) is malformed, contains
	// an invalid Sloth proof, references an incorrect draw block hash, or
	// uses T below the minimum threshold.
	ErrBadVDFResultTx
...
var errorCodeStrings = map[ErrorCode]string{
	...
	ErrBadDrawSettlementTx:       "ErrBadDrawSettlementTx",
	ErrSettlementTxWithoutVDFResult: "ErrSettlementTxWithoutVDFResult",
	ErrMissingDrawSettlementTx:    "ErrMissingDrawSettlementTx",
	ErrBadVDFResultTx:             "ErrBadVDFResultTx",
}
