...
const (
	// TxVersion is the standard transaction version for transfers and coinbases.
	TxVersion = 1

	// TxVersionPoolStake is the transaction version for pool staking (v0.2
	// canonical format). One TX per player. The 34-byte FDRP OP_RETURN carries
	// walletHash(20), amount(8), and drawHeight(4). Staked FDRW burns from
	// inputs; node credits declared amount to the pool balance tracked in
	// consensus state. Settlement mints the prize coinbase-style — O(1).
	TxVersionPoolStake = 6

	// TxVersionVDFResult is the transaction version for Sloth VDF proofs.
	// After draw block N connects, any node may post a v7 tx carrying the
	// 100_000-step modular-sqrt output over the draw block hash.  The first
	// valid VDFResult included in a block triggers settlement of draw N in the
	// same block.  This prevents miners from grinding the block hash to predict
	// draw outcomes — evaluating the VDF takes ~18-30 minutes of sequential
	// work, making pre-image search economically irrational.
	// OP_RETURN payload (272 bytes): "FDRV"(4) + drawHeight(4) + T(8) + output(256).
	TxVersionVDFResult = 7
