...
mempoolLoop:
	for _, txDesc := range sourceTxns {
		...
		// Pool stake txs are always includable: ValidatePoolStakeTx no longer
		// enforces a timing cutoff, and connectBlockStakingIndex remaps any
		// late stake to the next draw automatically.
	...
	// If VDF computation is complete for an unsettled draw, include the
	// VDFResult tx and (when pool balance > 0) the draw settlement tx.
	// VDFResult and settlement always land in the same block.
	if vdf := g.chain.GetPendingVDFResult(); vdf != nil {
		drawHeight := vdf.DrawHeight
		// Use ReconstructPoolBalance so the settlement is correct even if the
		// DB balance was zeroed by an old bug (RepairStrandedPoolBalances).
		poolBalance := g.chain.ReconstructPoolBalance(drawHeight)

		// Build settlement tx first so it sits at index 1 after coinbase.
		if poolBalance > 0 {
			stakes := g.chain.FetchPoolStakeEntries(drawHeight)
			result := blockchain.RunDraw(stakes, vdf.Seed)
			pd := blockchain.PoolDrawData{
				Balance:    poolBalance,
			...
			settleMsgTx := blockchain.BuildPoolDrawSettlementTx(drawHeight, pd)
			settleTx := btcutil.NewTx(settleMsgTx)
			blockTxns = append(blockTxns, settleTx)
			blockWeight += uint32(blockchain.GetTransactionWeight(settleTx))
			blockSigOpCost += int64(blockchain.CountSigOps(settleTx)) * blockchain.WitnessScaleFactor
			txFees = append(txFees, 0)
			txSigOpCosts = append(txSigOpCosts, 0)
			log.Debugf("VDF settlement: draw %d balance=%d jackpot=%v",
				drawHeight, poolBalance, result.JackpotWon)

			// VDF bounty: extra coinbase inflation to the submitter, only when
			// there is an actual draw (pool balance > 0).  Skip if submitter hash
			// is all-zeros (no mining address configured / tests).
			var zeroHash [20]byte
			if vdf.SubmitterWalletHash != zeroHash {
				bounty := blockchain.CalcVDFBounty(nextBlockHeight)
				bountyScript := blockchain.MakeWinnerP2WPKHScript(vdf.SubmitterWalletHash)
				coinbaseTx.MsgTx().AddTxOut(&wire.TxOut{
					Value:    bounty,
					PkScript: bountyScript,
				})
			}
		}

		// VDFResult tx (may be after settlement at any index ≥ 1).
		var drawBlockHashArr [32]byte
		copy(drawBlockHashArr[:], vdf.DrawBlockHash[:])
		vdfMsgTx := blockchain.BuildVDFResultTx(drawHeight, drawBlockHashArr, vdf.T, vdf.Output, vdf.SubmitterWalletHash)
		vdfTx := btcutil.NewTx(vdfMsgTx)
		blockTxns = append(blockTxns, vdfTx)
		blockWeight += uint32(blockchain.GetTransactionWeight(vdfTx))
		blockSigOpCost += int64(blockchain.CountSigOps(vdfTx)) * blockchain.WitnessScaleFactor
		txFees = append(txFees, 0)
		txSigOpCosts = append(txSigOpCosts, 0)
		log.Debugf("VDF: included VDFResult for draw %d (T=%d)", drawHeight, vdf.T)
	}
...
func (g *BlkTmplGenerator) UpdateBlockTime(msgBlock *wire.MsgBlock) error {
	...
	// Recalculate the difficulty for the new timestamp. This used to be
	// gated behind ReduceMinDifficulty (the old testnet-only rule), but
	// FalconDraw's EmergencyAdjustWindow also depends on the candidate
	// block's timestamp — and this is the only place a cached/reused
	// getblocktemplate response gets its timestamp advanced without a
	// full NewBlockTemplate regeneration. Skipping this call left the
	// emergency easing frozen at whatever it was during the last full
	// regen (only triggered by a new tip or a mempool change), instead of
	// live-ticking every poll the way the feature is meant to.
	// CalcNextRequiredDifficulty is a cheap, pure function that already
	// no-ops (returns the unchanged parent bits) when neither rule
	// applies, so calling it unconditionally is safe on every network.
...
// HasPendingVDF returns true if VDF computation has completed for an unsettled
// draw and a VDFResult tx is ready to be mined.  Used by the CPU miner to
// detect when a block template built before VDF completion has gone stale.
func (g *BlkTmplGenerator) HasPendingVDF() bool {
	return g.chain.GetPendingVDFResult() != nil
}
