...
const (
	...
	// according to the current default policy. FalconDraw uses 5MB blocks
	// (20M weight), so we allow transactions up to the full block weight.
	maxStandardTxWeight = 20000000
...
func CheckTransactionStandard(tx *btcutil.Tx, height int32,
	...
	// Pool staking TX (v6 / FDRP): validate format.
	if msgTx.Version == wire.TxVersionPoolStake {
		if err := blockchain.ValidatePoolStakeTx(tx, height, nil); err != nil {
			return txRuleError(wire.RejectNonstandard,
				fmt.Sprintf("invalid pool staking tx: %v", err))
		}
	}
