...
func dbFetchSpendJournalEntry(dbTx database.Tx, block *btcutil.Block) ([]SpentTxOut, error) {
	...
	// Mirror countSpentOutputs: exclude coinbase, VDFResult txs (sentinel-only
	// inputs, version 7), and any tx whose every input is a pool-draw sentinel.
	// Only txns with at least one real UTXO-spending input are passed to the
	// deserializer so the stxo count matches what was written at connect time.
	var blockTxns []*wire.MsgTx
	for _, tx := range block.MsgBlock().Transactions[1:] { // skip coinbase
		if tx.Version == wire.TxVersionVDFResult {
			continue
		}
...
func (b *BlockChain) createChainState() error {
	...
	err := b.db.Update(func(dbTx database.Tx) error {
		...
		// Create the buckets that store per-draw result metadata and VDF results.
		err = dbCreateDrawResultMetaBucket(dbTx)
		if err != nil {
			return err
		}
		err = dbCreatePoolStakeByHeightBucket(dbTx)
		if err != nil {
			return err
		}
		err = dbCreatePoolBalanceBucket(dbTx)
		if err != nil {
			return err
		}
		err = dbCreateVDFResultBucket(dbTx)
		if err != nil {
			return err
		}
		err = dbCreateStakeWalletCountBucket(dbTx)
		if err != nil {
			return err
		}
...
func (b *BlockChain) initChainState() error {
	...
	// Migrate: create drawresultmeta bucket if it doesn't exist yet.
	err = b.db.Update(func(dbTx database.Tx) error {
		if dbTx.Metadata().Bucket(drawResultMetaBucketName) == nil {
			return dbCreateDrawResultMetaBucket(dbTx)
		}
		return nil
	})
	if err != nil {
	...
	err = b.db.Update(func(dbTx database.Tx) error {
		if dbTx.Metadata().Bucket(poolStakeByHeightBucketName) == nil {
			return dbCreatePoolStakeByHeightBucket(dbTx)
		}
		return nil
	})
	if err != nil {
	...
	err = b.db.Update(func(dbTx database.Tx) error {
		if dbTx.Metadata().Bucket(poolBalanceBucketName) == nil {
			return dbCreatePoolBalanceBucket(dbTx)
		}
		return nil
	})
	if err != nil {
	...
	err = b.db.Update(func(dbTx database.Tx) error {
		if dbTx.Metadata().Bucket(vdfResultBucketName) == nil {
			return dbCreateVDFResultBucket(dbTx)
		}
		return nil
	})
	if err != nil {
	...
	err = b.db.Update(func(dbTx database.Tx) error {
		if dbTx.Metadata().Bucket(stakeWalletCountBucketName) == nil {
			return dbCreateStakeWalletCountBucket(dbTx)
		}
		return nil
	})
	if err != nil {
