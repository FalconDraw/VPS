...
// blockHasVDFResult returns true if the block template already contains a
// VDFResult tx (TxVersion 7).  Used to detect stale templates after VDF
// completion.
func blockHasVDFResult(msgBlock *wire.MsgBlock) bool {
	for _, tx := range msgBlock.Transactions {
		if blockchain.IsVDFResultTx(btcutil.NewTx(tx)) {
			return true
		}
	}
	return false
	...
	for extraNonce := uint64(0); extraNonce < maxExtraNonce; extraNonce++ {
		...
		for i := uint32(0); i <= maxNonce; i++ {
			...
			case <-ticker.C:
				...
				// The current block is stale if the VDF result for
				// an unsettled draw became available after this
				// template was built (template won't include the
				// VDFResult + settlement txs).
				if m.g.HasPendingVDF() && !blockHasVDFResult(msgBlock) {
					return false
				}

				m.g.UpdateBlockTime(msgBlock)
				// UpdateBlockTime can change header.Bits (FalconDraw's
				// EmergencyAdjustWindow eases difficulty based on the
				// new timestamp) — targetDifficulty was captured once
				// before this loop started, so it must be refreshed or
				// this solve loop keeps checking hashes against a stale,
				// harder target until the next full template rebuild.
				targetDifficulty = blockchain.CompactToBig(header.Bits)
			...
			header.Nonce = i
			hash := blockchain.BlockPoWHash(header)
			hashesCompleted++

			// The block is solved when the pow hash is less than
...
out:
	for {
		...
		if m.solveBlock(template.Block, blockHeight, ticker, quit) {
...
func (m *CPUMiner) GenerateNBlocks(n uint32) ([]*chainhash.Hash, error) {
	...
	for {
		...
		// a new block template can be generated.  When the return is
		// true a solution was found, so submit the solved block.
		if m.solveBlock(template.Block, blockHeight, ticker, nil) {
			block := btcutil.NewBlock(template.Block)
			m.submitBlock(block)
