...
static void generate_coinbase(ckpool_t *ckp, workbase_t *wb)
{
	...
	if (ckp->donvalid && ckp->donation > 0) {
		...
		wb->coinb2bin[wb->coinb2len++] = 2 + wb->insert_witness + wb->insert_vdf_bounty;
	} else
		wb->coinb2bin[wb->coinb2len++] = 1 + wb->insert_witness + wb->insert_vdf_bounty;
	...
	if (wb->insert_vdf_bounty) {
		/* FalconDraw: pay the VDF bounty to the submitter wallet hash
		 * detected in wb_merkle_bin_txns. Bounty is 100 FDRW pre-halving,
		 * 50 FDRW after height 840000 (blockchain.CalcVDFBounty). This is
		 * extra coinbase inflation on top of wb->coinbasevalue (subsidy+
		 * fees), so it must be a separate output, not folded into g64. */
		uint64_t bounty = (wb->height < 840000) ? 10000000000ULL : 5000000000ULL;

		u64 = htole64(bounty);
		memcpy(&wb->coinb3bin[wb->coinb3len], &u64, sizeof(uint64_t));
		wb->coinb3len += sizeof(uint64_t); // 8

		wb->coinb3bin[wb->coinb3len++] = 22; // P2WPKH scriptPubKey size
		wb->coinb3bin[wb->coinb3len++] = 0x00; // OP_0
		wb->coinb3bin[wb->coinb3len++] = 0x14; // OP_DATA_20
		memcpy(&wb->coinb3bin[wb->coinb3len], wb->vdf_bounty_hash, 20);
		wb->coinb3len += 20;
	}
				      ...
				      yyjson_val *txn_array, bool local)
{
	...
	wb->insert_vdf_bounty = false; /* recomputed fresh below, don't carry stale state */
	wb->merkles = 0;
	...
	if (wb->txns) {
		...
		for (i = 0; i < wb->txns; i++) {
			...
			/* FalconDraw: a VDFResult tx (version 7, fixed-format,
			 * exactly 390 bytes / 780 hex chars, no witness data)
			 * requires the coinbase to pay a VDF bounty to the
			 * submitter wallet hash embedded in its OP_RETURN. That
			 * hash sits in the fixed last 24 bytes of the tx, before
			 * the 4-byte locktime. Detect it here so the coinbase
			 * builder can add the required output. */
			if (len == 780 && !strncmp(txn, "07000000", 8)) {
				const char *walletHashHex = txn + len - 48;

				wb->insert_vdf_bounty = true;
				hex2bin(wb->vdf_bounty_hash, walletHashHex, 20);
			}
	      ...
	      const uchar *data, const uchar *hash, uchar *flip32, char *blockhash)
{
	...
	if (wb->insert_witness) {
		/* coinbase here is legacy-serialized (no marker/flag/witness),
		 * matching what miners hashed for the merkle root. But the
		 * coinbase's outputs already commit to a witness root (see
		 * gbt_witness_data), so the submitted block must carry the
		 * BIP141 witness serialization for the coinbase tx: marker
		 * (0x00), flag (0x01), and a single 32-byte zero witness
		 * item on the sole input, inserted around the unchanged
		 * legacy body. Without this, the coinbase's witness stack
		 * has 0 items and btcd's ValidateWitnessCommitment rejects
		 * the block outright. */
		char wcoinbase[1024];
		int wlen = 0;

		memcpy(wcoinbase, coinbase, 4); /* nVersion */
		wlen += 4;
		wcoinbase[wlen++] = 0x00; /* marker */
		wcoinbase[wlen++] = 0x01; /* flag */
		memcpy(wcoinbase + wlen, coinbase + 4, cblen - 4 - 4); /* txins/txouts */
		wlen += cblen - 4 - 4;
		wcoinbase[wlen++] = 0x01; /* witness stack: 1 item */
		wcoinbase[wlen++] = witness_nonce_size; /* item length: 32 */
		memcpy(wcoinbase + wlen, witness_nonce, witness_nonce_size);
		wlen += witness_nonce_size;
		memcpy(wcoinbase + wlen, coinbase + cblen - 4, 4); /* nLockTime */
		wlen += 4;

		__bin2hex(hexcoinbase, wcoinbase, wlen);
	} else {
